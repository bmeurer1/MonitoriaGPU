...
__global__ void matMul(float *matrizA, float *matrizB, float *matrizC, int linhasA, int linhasB, int colunasA, int colunasB){
	...
	//Aloca sub matrizes compartilhadas
	__shared__ float subMatrizA[BLOCK_SIZE][BLOCK_SIZE];
	__shared__ float subMatrizB[BLOCK_SIZE][BLOCK_SIZE];

	float sum = 0;

	for(int passo = 0; passo < colunasA; passo += BLOCK_SIZE){

		//Cada thread carrega um dos valores da matriz principal para a compartilhada
		subMatrizA[local_i][local_j] = matrizA[i * colunasA + (passo + local_j)];
		subMatrizB[local_i][local_j] = matrizB[(local_i + passo) * colunasB + j];

		__syncthreads();

		//Faz os cálculos com a matriz compartlilhada
		for(int k = 0; k < BLOCK_SIZE; k++){
			sum += subMatrizA[local_i][k] * subMatrizB[k][local_j];
		}

		__syncthreads();
	}

	//Joga os resultados de volta para a matriz na memória principal
...
//COMPARAÇÃO DE RESULTADOS
	...
	for(int i = 0; i < linhasA * colunasB; i++){
		if(fabs(h_matrizC[i] - h_matrizGPU[i]) > 0.1){
			erro = 1;
		}
	}
